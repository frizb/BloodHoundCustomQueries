...
parser = argparse.ArgumentParser(description='From a list of BloodHound users (must be in uppercase format like:'
                                             ' USER@DOMAIN.COM) this script will check the number of First Degree'
...
parser.add_argument("-csv", type=str, default="FirstDegreeLocalAdmins.csv",
...
if args.userlist != None:
    ...
    for user in input_file:
        with driver.session() as session:
            record_count = session.read_transaction(count_first_degree_local_admins, user.strip())
            if record_count > 0:
